...
class ScriptArguments:
    ...
    ddp_timeout: int = field(default=1800, metadata={"help": "ddp_timeout"})

    save_total_limit: int = field(default=None, metadata={"help": "save_total_limit"})

    trust_remote_code: bool = field(
        default=False, metadata={"help": "trust_remote_code"}
    )

    bf16: bool = field(default=False, metadata={"help": "bf16"})

    fp16: bool = field(default=False, metadata={"help": "fp16"})
    ...
    logging_dir: str = field(default=None, metadata={"help": "logging_dir"})

    output_dir: str = field(default=None, metadata={"help": "output_dir"})

    save_strategy: str = field(default="steps", metadata={"help": "save_strategy"})

    evaluation_strategy: str = field(
        default="steps", metadata={"help": "evaluation_strategy"}
    )
    metric_for_best_model: str = field(
        default="loss", metadata={"help": "metric_for_best_model"}
    )

    lr_scheduler_type: str = field(
        default="linear", metadata={"help": "lr_scheduler_type"}
    )

    eval_and_save_ratio_per_epoch: float = field(
        default=0.1, metadata={"help": "eval_and_save_ratio_per_epoch"}
    )

    num_train_epochs: float = field(default=3.0, metadata={"help": "num_train_epochs"})

    warmup_ratio: float = field(default=0.1, metadata={"help": "warmup_ratio"})

    logging_steps: float = field(default=500, metadata={"help": "logging_steps"})
    ...
    load_best_model_at_end: bool = field(
        default=False, metadata={"help": "load_best_model_at_end"}
    )

    prediction_loss_only: bool = field(
        default=False, metadata={"help": "prediction_loss_only"}
    )

    deepspeed: str = field(default=None, metadata={"help": "deepspeed"})
...
def compute_metrics(eval_preds):
    ...
    acc_mean = preds.mean()
    eval_dict = {"rewards/accuracies": acc_mean}
...
def main():
    ...
    tokenizer.pad_token_id = (
        tokenizer.pad_token_id if tokenizer.pad_token_id else tokenizer.unk_token_id
    )  # https://github.com/lm-sys/FastChat/blob/main/fastchat/train/train.py#L254
    ...
    ... ...
    """
    train_dataset = load_dataset("json", data_files=script_args.train_file)[
        "train"
    ].shuffle()
    eval_dataset = (
        load_dataset("json", data_files=script_args.validation_file)["train"].shuffle()
    ...
    eval_and_save_steps = int(
        t_total_per_epoch * script_args.eval_and_save_ratio_per_epoch
    )
    ...
    print_rank_0(
        f"per device train batch size: {training_args.per_device_train_batch_size}",
        global_rank,
    )
    print_rank_0(
    ...
    print_rank_0(
        f"eval and save ratio: {script_args.eval_and_save_ratio_per_epoch}", global_rank
    )
    print_rank_0(f"***", global_rank)
